...
abstract class WebSocket
{
    ...
    function __construct($addr, $port)
    {
        ...
        $this->logSucces("FlatFramewok Socket Server Starter");
        $this->logSucces("Listening on: $addr:$port");

    }

    public function logSucces($msg)
    {
        $colored_string = "";

        // Check if given foreground color found

        $colored_string .= "\033[0;32m";

        // Add string and end coloring
        $colored_string .= $msg . "\033[0m";

        $this->log($colored_string);
    }
            ...
            foreach ($read as $socket) {
                if ($socket == $this->master) {
                    ...
                    if ($client < 0) {
                        ...
                        $this->connect($client);
                        $this->logSucces("Client connected [x] => " . $client);
                    }
                } else {
                    ...
                    if ($numBytes === false) {
                        ...
                        $this->logError("Client disconnected. TCP connection lost: " . $socket);
                    } else {
    ...
    private function charAuto($string, $size)
    {
        $cn = strlen($string);
        $manque = $size - $cn;
        $final_string = $string;
